...
const char *topic_saldowNow = "Vimana_Electronic/KWH_Meter/KWH_1/Saldo_Now";    // add topic saldo now directory in broker server
const char *topic_topUpSaldo = "Vimana_Electronic/KWH_Meter/KWH_1/Topup_Saldo"; // add topic saldo top up directory in broker server
...
void connectWifi()
{
  ...
  {
    uint8_t _con_atempt = 0;
    WiFi.begin(wifi_ssid, wifi_password);
    while (WiFi.status() != WL_CONNECTED && _con_atempt <= 3)
    {
      _con_atempt++;
      delay(500);
    }
...
{
  int qos = 1;
  uint8_t _recon_attempt = 0;
  while (!client.connected() && _recon_attempt < 3)
  {
...
void routineTask()
{
  if (currentTimeMillis - lastTime > INTERVAL)
  {
    if (stateWifi == 0)
    {
      ...
      HwOut.turnOnLEDWIFI();
      client.loop();
      bool _status_pub = client.publish(topic_saldowNow, convertedSaldoNowToChar, true);
      // display status published
      String _dsp_stts_pub = _status_pub ? " Saldo Published" : "Not Published";
      lcd.clear();
      lcd.setCursor(0, 0);
      lcd.print(String(_dsp_stts_pub));
      delay(1000);
